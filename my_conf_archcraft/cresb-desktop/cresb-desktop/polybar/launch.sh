...
launch_bar() {
	...
	monRight=DP-1 #DELL
	monCenter=HDMI-0 #AZUs
	monLeft=HDMI-1 #SAMSUMG
	#Ejecucion con la variable de entorno MONITOR
	MONITOR=$monCenter polybar -q main -c "$DIR"/config.ini &
	MONITOR=$monLeft polybar -q left -c "$DIR"/config.ini &
	MONITOR=$monRight polybar -q right -c "$DIR"/config.ini &
}
